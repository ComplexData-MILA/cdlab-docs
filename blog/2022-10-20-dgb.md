...
2. $E_\mathrm{train} \cup E_\mathrm{test}$: Edges that are seen during training and reappear during test phase (can be considered *transductive* edges).
...
$$
\begin{equation}
\mathrm{novelty} = (1/ T) \sum_t (|E_t \ E_t^\mathrm{seen}|) / |E_t|
\end{equation}
$$
...
$$
\begin{align}
\mathrm{reocurrence} &= |E_\mathrm{train} ∩ E_\mathrm{test}| / |E_\mathrm{train}| \\
\mathrm{surprise} &= |E_\mathrm{test} \ E_\mathrm{train}| / |E_\mathrm{test}|
\end{align}
$$
